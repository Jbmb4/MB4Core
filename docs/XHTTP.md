...
Quando o painel publica uma alteração de perfil, o cliente recebe `MSG_CONFIG_UPDATE` (`0x14`) através de `DT_ACTION_ACTIVITY`; `MSG_APP_CONFIG_UPDATE` (`0x16`) é reservado para configurações internas do aplicativo. O receptor do runtime XHTTP trata os dois canais e converte o evento em `TUNNEL_SSH_RESTART_SERVICE`; antes de reconectar, `TunnelManagerThread` relê de `xhttp_demo_private` o servidor, a porta, o usuário, a senha, o SNI, o host, o caminho e a opção TLS. Assim, alterações de perfil feitas no painel passam a ser aplicadas sem exigir que o usuário encerre manualmente a conexão.
